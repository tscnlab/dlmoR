...
output_base <- "noisy_dlmo_trials"
dir_create(output_base)

# 1. Baseline only, no noise at 0min
baseline_dir <- file.path(output_base, "0min")
dir_create(baseline_dir)
...
write_csv(baseline_results, file.path(baseline_dir, "summary_0min.csv"))

# 2. Loop over non-zero jitter levels
all_results <- list()
for (sd_min in time_sd_levels[-1]) {
  t_sd_h <- sd_min / 60
  ...
  write_csv(res_df, file.path(subdir, glue("summary_{sd_min}min.csv")))
  all_results[[paste0(sd_min, "min")]] <- res_df
}

# optional: combine all non-zero levels
tcombined <- bind_rows(all_results, .id = "time_jitter")
write_csv(tcombined, file.path(output_base, "all_results_combined.csv"))
