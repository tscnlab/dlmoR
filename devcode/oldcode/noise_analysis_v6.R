...
# --- Loop through all profiles, save RDS ------------------------------------
run_all_profiles_full <- function(all_samples,
                                  out_dir        = "dlmo_results",
                                  n_rep          = 20,
                                  mel_sd         = 0.3,
                                  time_sd_main   = NULL,
  ...
  walk(all_samples, function(sample_dlmo) {
    df_res <- run_profile_noise_full(
      sample_dlmo     = sample_dlmo,
      n_rep           = n_rep,
      mel_sd          = mel_sd,
      time_sd_main    = time_sd_main,
      extra_time_sds  = extra_time_sds
    )
    fname <- file.path(out_dir,
                       paste0(sample_dlmo$profile_name, "_dlmo_full_boot.rds"))
    saveRDS(df_res, fname)
  })
}

# --- Run over all and save ----------------------------------------------

# To run on *only the first two* profiles with 2 reps each:
run_all_profiles_full(all_samples[1:2], out_dir = out_dir, n_rep = 2)

# (Make sure to remove or comment out any other calls to run_all_profiles_full.)

# --- Plotting Results -------------------------------------------------------- -------------------------------------------------------- -------------------------------------------------------- --------------------------------------------------------
# This example shows the change (delta) in DLMO relative to the clean estimate for each noise condition.
library(ggplot2)

# Read in all saved results
result_files <- list.files(out_dir, pattern = "_dlmo_full_boot\.rds$", full.names = TRUE)
all_res <- purrr::map_df(result_files, readRDS)

# Extract the clean (baseline) DLMO per profile
df_baseline <- all_res %>%
  filter(type == "clean") %>%
  select(profile_id, clean_dlmo = dlmo_est)

# Join and compute delta for all noisy runs
plot_df <- all_res %>%
  filter(type != "clean") %>%
  left_join(df_baseline, by = "profile_id") %>%
  mutate(
    delta      = dlmo_est - clean_dlmo,
    noise_group = dplyr::case_when(
      type == "mel_only"         ~ "Melatonin Only",
      type == "both_axes"        ~ "Melatonin + Time",
      grepl("time_sd", type)     ~ "Time Only",
      TRUE                         ~ "Other"
    )
  )

# Faceted boxplot of delta DLMO by noise group and condition
ggplot(plot_df, aes(x = type, y = delta)) +
  geom_boxplot() +
  facet_wrap(~ noise_group, scales = "free_x") +
  labs(
    title = "Impact of Noise on DLMO Estimate",
    x     = "Noise Condition",
    y     = "Delta DLMO (hours relative to clean)"
  ) +
  theme_minimal() +
  theme(
