...
analyze_and_plot_residual_clusters <- function(resid_vector, grid_df,
  ...
  # Add DLMO marker and label offset 45 degrees (top right) with arrow
  text_x <- dlmo_x + 0.1
  text_y <- dlmo_y + 0.1
  points(dlmo_x, dlmo_y, col = "cyan", pch = 18, cex = 2.5)  # filled diamond
  #arrows(dlmo_x, dlmo_y, text_x, text_y, col = "cyan", lwd = 2, length = 0.1)
  # text(
  #   text_x,
  #   text_y,
  ...
  shinyApp(
    ...
    server = function(input, output) {
      ...
      output$residStats <- renderUI({
        ...
        tagList(
          h4("Cluster Summary"),
          tags$p(strong("Entropy (spread):"), round(r$entropy, 3),
                 em("- higher = more diffuse residuals")),
