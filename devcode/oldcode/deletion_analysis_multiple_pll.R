...
with_progress({
  ...
  dir.create("deletion_partial_results_test", showWarnings = FALSE)
  results_list <- future_imap(profiles, function(df, id) {
    result <- safe_run(id, df)
    if (!is.null(result$result)) {
      saveRDS(result$result, file = paste0("deletion_partial_results_test/", id, ".rds"))
    }
    p()
    result
  })
...
# -----------------------------
saved_files <- list.files("deletion_partial_results_test", pattern = "\\.rds$", full.names = TRUE)
...
message("Partial results saved to 'deletion_partial_results_test/'")
...
write_csv(error_log, "dlmo_deletion_errors_test.csv")
