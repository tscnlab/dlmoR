...
plot_parallelogram <- function(plot, profile_data, pll_result) {
  ...
  x0 <- pll_result$pll_datetime_0
  x1 <- pll_result$pll_datetime_1
  ...
  x0_numeric <- posixct_to_decimal(x0, profile_data$datetime)
  x1_numeric <- posixct_to_decimal(x1, profile_data$datetime)
